...
class RWMB_Media_Field extends RWMB_Field
{
	...
	static function admin_enqueue_scripts()
	{
		...
		wp_localize_script( 'rwmb-media', 'i18nRwmbMedia', array(
			'add'                => apply_filters( 'rwmb_media_add_string', _x( '+ Add Media', 'media','xamin' ) ),
			'single'             => apply_filters( 'rwmb_media_single_files_string', _x( ' file', 'media','xamin' ) ),
			'multiple'           => apply_filters( 'rwmb_media_multiple_files_string', _x( ' files', 'media','xamin' ) ),
			'remove'             => apply_filters( 'rwmb_media_remove_string', _x( 'Remove', 'media','xamin' ) ),
			'edit'               => apply_filters( 'rwmb_media_edit_string', _x( 'Edit', 'media','xamin' ) ),
			'view'               => apply_filters( 'rwmb_media_view_string', _x( 'View', 'media','xamin' ) ),
			'noTitle'            => _x( 'No Title', 'media','xamin' ),
			'loadingUrl'         => RWMB_URL . 'img/loader.gif',
			'extensions'         => self::get_mime_extensions(),
			'select'             => _x( 'Select Files', 'media','xamin' ),
			'uploadInstructions' => _x( 'Drop files here to upload', 'media','xamin' )
		) );
	}
